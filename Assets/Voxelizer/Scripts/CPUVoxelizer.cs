...
namespace VoxelSystem
{
  ...
  public class CPUVoxelizer
  {
    ...
    public static void Voxelize(Mesh mesh, int resolution, out List<Voxel_t> voxels, out float unit)
    {
      ...
      Voxelize(mesh, mesh.bounds, resolution, out voxels, out unit, out _);
    }

    // http://blog.wolfire.com/2009/11/Triangle-mesh-voxelization
    public static void Voxelize(Mesh mesh, Bounds bounds, int resolution, out List<Voxel_t> voxels, out float unit, out bool[,,] vMap)
    {
      mesh.RecalculateBounds();

      //var bounds = mesh.bounds;
      //var bounds = new Bounds(Vector3.zero, 20f * Vector3.one);
      ...
      vMap = new bool[width, height, depth];
